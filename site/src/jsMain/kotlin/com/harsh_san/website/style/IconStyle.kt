package com.harsh_san.website.style
        ...
        Modifier
            ...
            .background(
                if (colorMode.isLight)
                    Res.Theme.SOCIAL_ICON_BACKGROUND_LIGHT.color else Res.Theme.SOCIAL_ICON_BACKGROUND_DARK.color
            )
    }
}
...
val ThemeIconStyle by ComponentStyle {
    base {
        Modifier
            .background(
                if (colorMode.isLight)
                    Res.Theme.DARK_BLUE.color else Colors.White
            )
            .transition(
                CSSTransition(property = "background", duration = 300.ms)
            )
